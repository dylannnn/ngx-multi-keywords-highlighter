...
LIBRARY_PACKAGE_JSON_FILE="libs/ngx-multi-keywords-highlighter/package.json"
...
jq --arg NEXT_RELEASE_VERSION "$NEXT_RELEASE_VERSION" '.version = $NEXT_RELEASE_VERSION' "$LIBRARY_PACKAGE_JSON_FILE" > "tmp-lib-package.json" && mv "tmp-lib-package.json" "$LIBRARY_PACKAGE_JSON_FILE"

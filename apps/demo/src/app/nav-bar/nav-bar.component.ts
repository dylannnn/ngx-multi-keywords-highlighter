...
export class NavBarComponent implements OnInit {
      ...
      .subscribe({
        next: (onNavigationEnd) => {
          const navigationEndEvent =
            onNavigationEnd as unknown as NavigationEnd;
          const mkhOutletsURLRegex = new RegExp(
            /(multi-keywords-highlighter:lib-experimental)/g,
          );
          const onMkhOutletURL = mkhOutletsURLRegex.test(
  ...
  toggleLib(): void {
    ...
    this.showHighlighter$.pipe(take(1)).subscribe({
      next: (status) => {
        if (!status) {
          this.clearNavigationOutlet();
        } else {
          this.router.navigate([
            { outlets: { 'multi-keywords-highlighter': ['lib-experimental'] } },
          ]);
        }
      },
      error: () => {
